...
- [x] Добавить миксин `disabled: true` для всех слотов, в которых нет событий, но их дата прошла. А так же разделять слоты на двое по текущей дате.
- [x] Добавить миксин `disabled: true` для комнат у которых все слоты уже заняты или время уже вышло.
- [ ] Поправить в Safari открытие `tooltip`. Реализовать это через JS.
- [ ] Добавить обновление данных `apollo` после перехода со страницы фомы на главную, а также после переключения дат и каждые 60 сек. Чтобы при переходе на главную страницу данные уже были обновленные данные. Сейчас нужно ждать 1000мс, и с этим интервалом он будет всегда подгружать эти данные. А так же при переключении даты, при попадании на обновление, слоты немного моргают.
